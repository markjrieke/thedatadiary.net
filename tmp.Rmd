...
3. See if I can recover the parameters with a model in Stan.
...
```{r raccoon-01-post-fit, eval=FALSE}
raccoon_01@stanfit %>%
...
```{r raccoon-03-post-fit, eval=FALSE}
raccoon_03@stanfit %>%
...
```{r raccoon-04-precis, eval=FALSE}
precis(raccoon_04, depth = 2)
```

## Eploring the posterior
...
```{r setup-counterfactual, eval=FALSE}
...
* `raccoon_02` added terms for unit the patient visited and age. This recovered the parameters we set manually, but didn't pool any information across units and only allowed us to draw inferences from the units in the dataset.
