...
If you want to skip the derivation that follows in the rest of the article, this sufficient gamma density function that can just be dropped into a function block at the top of a Stan model. Note that the gamma distribution has, like, a thousand different parameterizations. This, and other gamma distributions throughout this article, make use of the shape ($\alpha$) / scale ($\theta$) parameterization.
...
$$
\log(\Pr(y,n|\alpha,\theta)) = -n\left(\Gamma(\alpha)+\alpha\log(\theta)\right) + (\alpha-1)\sum\log(y) - \sum y/\theta
$$
